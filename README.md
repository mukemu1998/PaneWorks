...
artifacts/
  releases/
    v0.2.2/
      PaneWorks-v0.2.2-win-x64-portable.zip
      PaneWorks-v0.2.2-win-x64-portable.zip.sha256
      PaneWorks-v0.2.2-win-x64-portable/
        PaneWorks.exe
        README.md
        VERSION
```
...
当前版本为 `0.2.2`，处于原型打磨与持续迭代阶段。用户自建布局、工作区方案与个人设置保存在 `%AppData%\PaneWorks\`，不会写入仓库源码目录。
