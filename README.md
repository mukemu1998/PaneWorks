...
<p align="center">
  ...
  <a href="./docs/development-guide.zh-CN.md">开发规范</a>
</p>
...
<p align="center">
  ...
  <img alt="Version 0.3.1" src="https://img.shields.io/badge/Version-0.3.1-22c55e" />
  <img alt="License MIT" src="https://img.shields.io/badge/License-MIT-eab308" />
</p>
...
| 快捷键吸附 | 按住 `Shift` 使用当前初始布局吸附；按住 `Ctrl + Shift` 可在本次会话中进行临时插入与联动。 |
| 运行时联动调整 | 已吸附窗口支持共享边缘联动调整，不改写原始吸附布局。 |
| 临时拓扑吸附 | `Ctrl + Shift` 拖入区域边缘可二等分插入，拖到独占分割线可三等分插入；落位会预显示，结果仅在当前运行会话生效。 |
| 工作区方案 | 可把分区布局与指定窗口绑定，支持已打开窗口重新自动吸附、未打开软件自动启动后自动吸附。 |
| 文件夹路径绑定 | Explorer 文件夹窗口可按路径记住对应区域，适合固定项目目录工作流。 |
| 自包含发布 | 提供 Windows 自包含便携包，普通用户无需额外安装 .NET Runtime。 |
| 自动更新 | 可在启动时检查 GitHub Release；确认后自动下载、校验并覆盖更新。 |
...
artifacts/
  releases/
    v0.3.1/
      PaneWorks-v0.3.1-win-x64-portable.zip
      PaneWorks-v0.3.1-win-x64-portable.zip.sha256
      PaneWorks-v0.3.1-win-x64-portable/
        PaneWorks.exe
        README.md
        LICENSE
        VERSION
...
- 功能代码按 `App / Core / Infrastructure` 分层：界面交互留在 `PaneWorks.App`，布局模型与计算放在 `PaneWorks.Core`，Win32、持久化和系统集成放在 `PaneWorks.Infrastructure`。
- 主菜单、浮窗和二级弹窗定位已经拆到 `MainWindow.Workbench.cs`，避免继续把外壳 UI 逻辑堆回主窗口主文件。
- 后续开发严格遵守 [PaneWorks 开发规范](./docs/development-guide.zh-CN.md)：新功能先确定层级和落点，再实现、构建、覆盖测试包。
- `artifacts/`、`bin/`、`obj/`、个人布局和工作区配置都属于本地运行或构建产物，不进入源码提交。

## 当前版本

当前公开版本为 `0.3.1`。用户自建布局、工作区方案与个人设置保存在 `%AppData%\PaneWorks\`，不会写入仓库源码目录。
